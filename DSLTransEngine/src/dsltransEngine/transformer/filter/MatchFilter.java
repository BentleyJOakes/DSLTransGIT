...
import dsltransEngine.transformer.TermProcessor;
import dsltransEngine.transformer.TransformationController;
...
public class MatchFilter {
	...
	private dsltrans.MatchModel _matchModel;
	...
	@SuppressWarnings("rawtypes")
	public MatchFilter(TransformationRule tr, dsltrans.Rule rule, dsltrans.MatchModel mm, InstanceDatabaseManager instanceDatabaseManager) {
		...
		for (Object obj : mm.getClass_()) {
			dsltrans.MatchClass mc = (dsltrans.MatchClass) obj;
		...
		List<dsltrans.ApplyClass> ac = new LinkedList<dsltrans.ApplyClass>();
		for(Object obj : rule.getBackwards()) {
			dsltrans.AbstractTemporalRelation br = (dsltrans.AbstractTemporalRelation) obj;
		...
		for (Object obj : mm.getAssociation()) {
			dsltrans.MatchAssociation ma = (dsltrans.MatchAssociation) obj;
	...
	private boolean shouldBeBinded(dsltrans.MatchAssociation ma) {
		return !(ma instanceof dsltrans.MatchMayBeSameRelation);
	}
	...
	private boolean CreateRelations(InstanceDatabase matchModel,MetaModelDatabase matchMetaModel
		...
		for(MatchRelationFilter rf : getMatchRelationFilters()) {
			...
			if(rf.getFilterDatabase().isEmpty() && rf.getAssociation() instanceof dsltrans.PositiveMatchAssociation)
				return false;
	...
	public static boolean isPositive(dsltrans.MatchAssociation association) {
		return association instanceof dsltrans.PositiveMatchAssociation || association instanceof dsltrans.PositiveIndirectAssociation || association instanceof dsltrans.MatchMayBeSameRelation;
	}

	public static boolean isPositive(dsltrans.AbstractTemporalRelation association) {
		return association instanceof dsltrans.PositiveBackwardRestriction;
	}	
	
	public static boolean isAnyPositive(dsltrans.MatchClass mclass) {
		return mclass instanceof dsltrans.AnyMatchClass;
	}
	
	public static boolean isPositive(dsltrans.MatchClass matchClass) {
		return matchClass instanceof dsltrans.PositiveMatchClass;
	}
	
	private boolean isExistPositive(dsltrans.MatchClass target) {
		return target instanceof dsltrans.ExistsMatchClass;
	}
	
	private boolean isNegative(dsltrans.MatchClass matchClass) {
	...
	private boolean onlyNegativeTemporals(ApplyEntityFilter ef) {
		{  // temporal relations
			...
			while(iter.hasNext()) {
				...
				if(rf.getAssociation() instanceof dsltrans.PositiveBackwardRestriction) {
	...
	private boolean isIndirect(dsltrans.MatchAssociation association) {
		return association instanceof dsltrans.PositiveIndirectAssociation ||
		association instanceof dsltrans.NegativeIndirectAssociation;
	}
		...
		{ // regular match entities
			...
			while(iter.hasNext()) {
				...
				if (isPositive(ef.getMatchClass()) && !exist(positiveMatchAndApplyEntitiesIDList,ef.getId())) {
					assert !(ef.getMatchClass() instanceof dsltrans.ApplyClass);
		...
		{ // referenced apply entities from backward restrictions
			...
			while(iter.hasNext()) {
				...
				if (!exist(positiveMatchAndApplyEntitiesIDList,ef.getId())) {
					assert ef.getApplyClass() instanceof dsltrans.ApplyClass;
					positiveMatchAndApplyEntitiesIDList.add(ef.getId());
				}
			};
		}		
		{ // regular match associations
			...
			while(iter.hasNext()) {
				...
				if(isPositive(rf.getAssociation())
					...
					String targetId = getIdFrom(rf.getTargetClass());
					if(rf.getAssociation() instanceof dsltrans.PositiveMatchAssociation) {
		...
		{ // indirect match associations
			...
			while(iter.hasNext()) {
				...
				if(isPositive(rf.getAssociation())
					...
					if(rf.getAssociation() instanceof dsltrans.PositiveIndirectAssociation) {
						if(!first)
							queryBody += ",";
						first = false;						
//queryBody += neighbourFact+"("+rf.getId()+","+ sourceId +","+ targetId +")";
		...
		
		{
			...
			for (dsltrans.AbstractAttributeRelation aatr: getOwnAttributeRelations()) {
	...
	private String getPredicatesFromAttributeRelation(
			dsltrans.AbstractAttributeRelation aatr) {
	...
	private List<dsltrans.AbstractAttributeRelation> getOwnAttributeRelations() {
		List<dsltrans.AbstractAttributeRelation> res = new LinkedList<dsltrans.AbstractAttributeRelation>();
		for (Object obj: _transformationRule.getRule().getAttributeRelations()) {
	...
	private boolean mayBeSameRelations(String idi, String idj) {
		...
		dsltrans.MatchClass mc_i = findMatchClass(idi);
		dsltrans.MatchClass mc_j = findMatchClass(idj);
		
		if (mc_i != null && mc_j != null) {
			return _transformationRule.hasMayBeSameRelation(mc_i, mc_j);
		}

		dsltrans.ApplyClass ac_i = findApplyClass(idi);
		dsltrans.ApplyClass ac_j = findApplyClass(idj);
	...
	private dsltrans.ApplyClass findApplyClass(String idi) {
		for(ApplyEntityFilter ef : getApplyEntityFilters()) {
	...
	private dsltrans.MatchClass findMatchClass(String idi) {
		for(MatchEntityFilter ef : getMatchEntityFilters()) {
			if (ef.getId().equals(idi)) {
...
//queryBody += neighbourFact+"("+rf.getId()+","+ sourceId +","+ targetId +")";
	...
	private String getIdFrom(dsltrans.ApplyClass targetClass) {
		for(ApplyEntityFilter ef : getApplyEntityFilters()) {
...
//queryBody += neighbourFact+"("+rf.getId()+","+ sourceId +","+ targetId +")";
	...
	private void getQueryBodyByRelation(List<String> queryBodyList) {
		{
			...
			while(iter.hasNext()) {
				...
				if(rf.getAssociation() instanceof dsltrans.NegativeMatchAssociation
						&&
				...
				} else if(rf.getAssociation() instanceof dsltrans.NegativeIndirectAssociation
						&&
						isPositive(rf.getSourceClass()) &&
						isPositive(rf.getTargetClass())
						) {
		...
		{  // temporal relations
			...
			while(iter.hasNext()) {
				TemporalRelationFilter rf = iter.next();
				if(rf.getAssociation() instanceof dsltrans.NegativeBackwardRestriction
						&&
						isPositive(rf.getSourceClass())		
				) {
					String sourceId = getIdFrom(rf.getSourceClass());
	...
	private List<MatchRelationFilter> getNegativeRelations(dsltrans.MatchClass matchClass) {
	...
	private List<MatchRelationFilter> getNegativeIndirectRelations(dsltrans.MatchClass matchClass) {
		List<MatchRelationFilter> result = new LinkedList<MatchRelationFilter>();
		
		Iterator<MatchRelationFilter> iter = getMatchRelationFilters().iterator();
		while(iter.hasNext()) {
			MatchRelationFilter rf = iter.next();
			if(rf.getAssociation() instanceof dsltrans.NegativeIndirectAssociation
					&& (rf.getSourceClass() == matchClass || rf.getTargetClass() == matchClass))
				result.add(rf);
		}
		return result;
	...
	private List<MatchRelationFilter> getNegativeTemporalRelations(dsltrans.MatchClass matchClass) {
		List<MatchRelationFilter> result = new LinkedList<MatchRelationFilter>();
	...
	private String getIdFrom(dsltrans.MatchClass sourceClass) {
		for(MatchEntityFilter ef : getMatchEntityFilters()) {
	...
	public AbstractFilter getFilter(dsltrans.MatchClass match) {
		for(MatchEntityFilter ef : getMatchEntityFilters()) {
	...
	public void set_matchModel(dsltrans.MatchModel _matchModel) {
		this._matchModel = _matchModel;
	}


	public dsltrans.MatchModel get_matchModel() {
		return _matchModel;
	}
	...
	public ApplyAttributeFilter findApplyAttributeFilter(dsltrans.Attribute a) {
